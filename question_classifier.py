...
class QuestionClassifier:
    def __init__(self):
        ...
        # 加载特征词
        enc = 'utf-8'
        self.disease_wds= [i.strip() for i in open(self.disease_path, 'r', encoding=enc) if i.strip()]
        self.department_wds= [i.strip() for i in open(self.department_path, 'r', encoding=enc) if i.strip()]
        self.check_wds= [i.strip() for i in open(self.check_path, 'r', encoding=enc) if i.strip()]
        self.drug_wds= [i.strip() for i in open(self.drug_path, 'r', encoding=enc) if i.strip()]
        self.food_wds= [i.strip() for i in open(self.food_path, 'r', encoding=enc) if i.strip()]
        self.producer_wds= [i.strip() for i in open(self.producer_path, 'r', encoding=enc) if i.strip()]
        self.symptom_wds= [i.strip() for i in open(self.symptom_path, 'r', encoding=enc) if i.strip()]
        ...
        self.deny_words = [i.strip() for i in open(self.deny_path, 'r', encoding=enc) if i.strip()]
        # 构造领域actree
